...
class get():
    ...
    def __init__(self, file):
        ...
        for n in self.accounts.keys():
            # 获取key
            self.keys.append(self.accounts[n])
            cmd = "cd " + os.path.join(self.client_path, self.accounts[n]) + " &&" + " ".join(("cat", "address"))
        ...
        # 当前client的地址
        cmd = "cd " + os.path.join(self.client_path, "data/keys") + " &&" + " ".join(("cat", "address"))
        err, result = subprocess.getstatusoutput(cmd)
        assert err == 0, result
