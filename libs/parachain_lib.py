...
class ParaChain(object):
    ...
    # 创建平行链
    def CreateChain(self, chain_conf, **kwargs):
        """
        chain_conf：平行链的配置
        """
        chain_conf = "\'" + chain_conf + "\'"
        res = ["xkernel", "invoke", "'$parachain'", "--method", "createChain", "-a", chain_conf]
    ...
    # 查询平行链的群组
    def QueryChainGroup(self, name, **kwargs):
        """
        name: 平行链名
        """
        args = {
    ...
    # 修改平行链的群组
    def EditChainGroup(self, name, admin, **kwargs):
        """
        name: 平行链名
        admin 群组成员
        """
    ...
    # 停用平行链
    def StopChain(self, name, **kwargs):
        """
        name: 平行链名
        """
        args = {
    ...
    def GenChainConf(self, consensus, name, group=False, group_name=""):
        """
        ...
        if consensus == "pow":
            ...
            cons_data["genesis_consensus"] = { 
                "name": "single", 
                "config": {
                    "miner": self.conf.addrs[0], 
                    "period": "3000" 
                } 
            }  
        elif consensus == "tdpos" or consensus == "xpos":
            cons_data["genesis_consensus"] = { 
                "name": "tdpos", 
                "config": { 
        ...
        elif consensus == "poa" or consensus == "xpoa":
            cons_data["genesis_consensus"] = { 
                "name": "xpoa", 
                "config": { 
                    "version": "1",
    ...
    #查询所有平行链
    def QueryParachain(self, **kwargs):
        """
        不包括xuper
        """
        s = ("status", "-L")
