"""
说明: 测试java native合约 部署、调用、查询、升级的异常场景
"""
import json
import pytest
import time
...
class TestJavaNative():
    ...
    def test_case01(self, input):        
        ...
        err, result = input.test.xlib.DeployContract("native", "java", self.cname, self.file, contract_account, args)
        assert err != 0, "部署java native合约成功，不合预期： " + result
        msg = "already exists"
    ...
    def test_case02(self, input):
        ...
        err, result = input.test.xlib.DeployContract("native", "java", self.cname, self.file, address, args)
        assert err != 0, "部署java native合约成功，不合预期： " + result
        msg = "Key not found"   
        assert msg in result, "报错信息错误"
    ...
    def test_case03(self, input):
        ...
        print("\n使用不存在的合约账号，部署合约") 
        args = json.dumps(self.deploy)
        err, result = input.test.xlib.DeployContract("native", "java", self.cname, self.file, self.ca, args)
        assert err != 0, "部署java native合约成功，不合预期： " + result
        msg = "Key not found"
        assert msg in result, "报错信息错误"
