...
class TestCppWasmErr():
    ...
    def test_case01(self, input):        
        ...
        input.test.xlib.DeployContract("wasm", "cpp", self.cname, self.file, contract_account, args)
    ...
    def test_case02(self, input):
        """使用普通账号部署合约"""
        print("\n使用普通账号部署合约")
        address = input.addrs[0]
        args = json.dumps(self.deploy)
        err, address = input.test.xlib.GetAddress("data/keys")
        err, result = input.test.xlib.DeployContract("wasm", "cpp", self.cname, self.file, address, args)
        assert err != 0, "部署cpp wasm合约成功，不合预期： " + result
        msg = "Key not found"
        assert msg in result, "报错信息错误"
    ...
    def test_case03(self, input):
        ...
        print("\n使用不存在的合约账号，部署合约")
        args = json.dumps(self.deploy)
        err, result = input.test.xlib.DeployContract("wasm", "cpp", self.cname, self.file, self.ca, args)
        assert err != 0, "部署cpp wasm合约成功，不合预期： " + result
        msg = "Key not found"
        assert msg in result, "报错信息错误"
    ...
    def test_case04(self, input):
        """升级合约，用非部署账号"""
        print("\n升级合约，用非部署账号")
        err, result = input.test.xlib.UpgradeContract("wasm", self.cname, self.file, self.ca)
    ...
    def test_case05(self, input):
        ...
        print("\n升级合约，用部署账号对应的普通账号")
        err, address = input.test.xlib.GetAddress("data/keys")
        err, result = input.test.xlib.UpgradeContract("wasm", self.cname, self.file, address)
        assert err != 0, "升级cpp wasm合约成功，不合预期： " + result
        msg = "verify contract owner permission failed"
        assert msg in result, "报错信息错误"
    ...
    def test_case06(self, input):
        ...
        print("\n升级不存在的合约")
        contract_account = "XC" + input.account + "@" + input.conf.name
        err, result = input.test.xlib.UpgradeContract("wasm", self.cname + "err", self.file, contract_account)
        assert err != 0, "升级cpp wasm合约成功，不合预期： " + result
    ...
    def test_case07(self, input):
        ...
        contract_account = "XC" + input.account + "@" + input.conf.name
        args = json.dumps(self.deploy)
        err, result = input.test.xlib.DeployContract("native", "cpp", self.cname + "new", \
            self.file, contract_account, args)
        assert err != 0, "部署cpp wasm合约成功，不合预期： " + result
    ...
    def test_case08(self, input):
        ...
        contract_account = "XC" + input.account + "@" + input.conf.name
        args = json.dumps(self.deploy)
        err, result = input.test.xlib.DeployContract("wasm", "go", self.cname + "new", \
            self.file, contract_account, args)
        assert err != 0, "部署cpp wasm合约成功，不合预期： " + result
    ...
    def test_case09(self, input):
        ...
        contract_account = "XC" + input.account + "@" + input.conf.name
        args = json.dumps(self.deploy)
        err, result = input.test.xlib.DeployContract("wasm", "java", self.cname + "new", \
            self.file, contract_account, args)
        assert err != 0, "部署cpp wasm合约成功，不合预期： " + result
        msg = "bad runtime"
        assert msg in result, "报错信息错误"
    ...
    def test_case10(self, input):
        ...
        err, result = input.test.xlib.DeployContract("wasm", "cpp", self.cname + "new", \
            java_file, contract_account, args)
        assert err != 0, "部署cpp wasm合约成功，不合预期： " + result
    ...
    def test_case13(self, input):
        ...
        err, result = input.test.xlib.InvokeContract("wasm", self.cname + "err", "increase", args)
