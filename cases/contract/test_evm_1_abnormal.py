...
class TestEVM():
    ...
    def test_case01(self, input):         
        ...
        input.test.xlib.DeployContract("evm", "", self.cname, \
    ...
    def test_case02(self, input):   
        ...
        err, result = input.test.xlib.DeployContract("evm", "", self.cname + "new", \
            self.file, address, args, abi = self.abi)         
        assert err != 0, "部署evm合约成功，不合预期： " + result
        msg = "Key not found"   
        assert msg in result, "报错信息错误"
    ...
    def test_case03(self, input):   
        ...
        args = json.dumps(self.deploy)   
        err, result = input.test.xlib.DeployContract("evm", "", self.cname, \
            self.file, self.ca, args, abi = self.abi)         
        assert err != 0, "部署evm合约成功，不合预期： " + result
        msg = "Key not found"
        assert msg in result, "报错信息错误"